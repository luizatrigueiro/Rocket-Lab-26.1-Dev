...
def atualizar_produto(id_produto: str, produto_update: ProdutoUpdate, db: Session = Depends(get_db)):
    produto = db.query(Produto).filter(Produto.id_produto == id_produto).first()
    if not produto:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, 
    ...
    atualizacao_dados = produto_update.model_dump(exclude_unset=True)
    for chave, valor in atualizacao_dados.items():
